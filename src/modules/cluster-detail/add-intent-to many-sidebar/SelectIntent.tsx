...
function FormikSelectIntent({
  ...
  const [{ isOpen, data: mesaggesIdList }, openSidebar] = useRecoilState(sidebarState)
  const intentOptions = intents.map(intent => ({
    label: intent.name,
    value: intent.id
  }))
  const [actionState, submit] = useAsyncAction(async (values, list) => {
    const savedData = await Promise.all(
      list.map(async (id: string) => {
        return await SelectionService.createSelection({
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => submit(values, mesaggesIdList),
